...
def email_normalize_all(email_str):
    """ Sanitize and standardize email address entries.
        A normalized email is considered as :
        - having a left part + @ + a right part (the domain can be without '.something')
        - being lower case
        ...
        Unlike email_normalize, this method can return multiple normalized emails.

    :param str email_str: email address or addresses
    :return: list of normalized emails or None if no valid email found
    :rtype: list or None
    """
    if not email_str:
        return None
    
    emails = email_split(email_str)
    if not emails:
        return None
    ...
    return normalized_emails if normalized_emails else None
